...
int main() {
  ...
  // Performance metrics
  int64_t runtime = y-x; //get_timer();
  int64_t performance = 2.0 * F * F * M * N / runtime;
  int64_t utilization = 100 * performance / (2.0 * NR_LANES);

  printf("The execution took %ld cycles.\n", runtime);
  printf("The performance is %ld OP/cycle (%ld%% utilization).\n", 
